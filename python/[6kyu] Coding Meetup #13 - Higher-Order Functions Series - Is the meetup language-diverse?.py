...
def is_language_diverse(lst):
    language_counts = Counter(developer["language"] for developer in lst)
    sorted_count = list(sorted(language_counts.items(), key = lambda item: item[1]))

    return True if sorted_count[0][1] * 2 >= sorted_count[-1][1] else False
