...
def sort_twisted37(arr):
    table = str.maketrans("37","73")
    translated_arr = [[num, int(str(num).translate(table))] for num in arr]
    translated_arr.sort(key = lambda x: x[1])

    return [nums[0] for nums in translated_arr]
